// File:          slam_ros_test.cpp
// Date:          02/04/2021
// Description:   A ros controller to map the track using gmapping-heavily adapted from Cybertronics pioneer3at.cpp controller
// Author:        Stephanie Liaw?Cybertronics
// Modifications: Modified for 2 wheel drive ?E-puck instead of 4 wheel drive
...
void updateSpeed(){
    ...
    if (obstacle > OBSTACLE_THRESHOLD) {
        ...
        speeds[0] = speedFactor * rightObstacle;
        speeds[1] = speedFactor * leftObstacle;
    } else {
        speeds[0] = MAX_SPEED;
        speeds[1] = MAX_SPEED;
    }
    ...
    for (int i = 0; i < NMOTORS; ++i) {
        ...
        set_velocity_client = n->serviceClient<webots_ros::set_float>(std::string("slam_ros_test/") + std::string(motorNames[i]) +
...
void broadcastTransform() {
    ...
    br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "base_link", "slam_ros_test/Sick_LMS_291"));
}
...
void quit(int sig) {
  ROS_INFO("User stopped the 'slam_ros_test' node.");
...
int main(int argc, char **argv) {
    ...
    // create a node named 'slam_ros_test' on ROS network
    ros::init(argc, argv, "slam_ros_test", ros::init_options::AnonymousName);
    ...
    timeStepClient = n->serviceClient<webots_ros::set_int>("slam_ros_test/robot/time_step");
    ...
    for (int i = 0; i < NMOTORS; ++i) {
        ...
        set_position_client = n->serviceClient<webots_ros::set_float>(std::string("slam_ros_test/") + std::string(motorNames[i]) +
    ...
    set_lidar_client = n->serviceClient<webots_ros::set_int>("slam_ros_test/Sick_LMS_291/enable");
    lidar_srv.request.value = TIME_STEP;
    if (set_lidar_client.call(lidar_srv) && lidar_srv.response.success) {
        ROS_INFO("Lidar enabled.");
        sub_lidar_scan = n->subscribe("slam_ros_test/Sick_LMS_291/laser_scan/layer0", 10, lidarCallback);
    ...
    set_GPS_client = n->serviceClient<webots_ros::set_int>("slam_ros_test/gps/enable");
    GPS_srv.request.value = 32;
    if (set_GPS_client.call(GPS_srv) && GPS_srv.response.success) {
        sub_GPS = n->subscribe("slam_ros_test/gps/values", 1, GPSCallback);
    ...
    set_inertial_unit_client = n->serviceClient<webots_ros::set_int>("slam_ros_test/inertial_unit/enable");
    inertial_unit_srv.request.value = 32;
    if (set_inertial_unit_client.call(inertial_unit_srv) && inertial_unit_srv.response.success) {
        sub_inertial_unit = n->subscribe("slam_ros_test/inertial_unit/quaternion", 1, inertialUnitCallback);
    ...
    set_accelerometer_client = n->serviceClient<webots_ros::set_int>("slam_ros_test/accelerometer/enable");
    ...
    set_camera_client = n->serviceClient<webots_ros::set_int>("slam_ros_test/camera/enable");
    ...
    set_gyro_client = n->serviceClient<webots_ros::set_int>("slam_ros_test/gyro/enable");
    ...
    ROS_INFO("You can now start the creation of the map using 'rosrun gmapping slam_gmapping "
            "scan:=/slam_ros_test/Sick_LMS_291/laser_scan/layer0 _xmax:=30 _xmin:=-30 _ymax:=30 _ymin:=-30 _delta:=0.2'.");
